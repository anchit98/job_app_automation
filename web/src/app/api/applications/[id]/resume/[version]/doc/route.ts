...
export async function GET(
  ...
  if (!resumeVersion?.drive_doc_id || resumeVersion.status !== "ready") {
    return NextResponse.json({ error: "Editable doc not found" }, { status: 404 });
  }

  try {
    const auth = await getGoogleAuthClient();
    const drive = new DriveClient(auth);
    const link = await drive.getWebViewLink(resumeVersion.drive_doc_id);
    if (!link) {

...
- **Auth & Identity** — Google OAuth flows (no app login)
- **Data & Persistence** — SQLite writes, JSON validation
...
### 1.1 Auth & Local Trust

- **Someone else on the same machine opens `localhost:3000`.**
  - *Symptom:* Full access to the app and Google-connected workflows.
  - *Mitigation:* Document that this is a single-user local install on a trusted machine. No app login by design.
...
- **Google Drive/Gmail API is down or degraded.**
  ...
  - *Mitigation:* Retry with exponential backoff (max 3 attempts). If still failing, mark the artifact as `pending_upload`, keep the generated bytes in SQLite (or in the response payload), and expose a "Retry upload" button. Never lose generated content.
  - *Severity:* High.

- **SQLite database file corrupted or locked.**
  - *Symptom:* Server actions throw database errors.
  - *Mitigation:* WAL mode + single-writer pattern. Document backup of `data/app.db`. On corruption, restore from backup or delete and re-migrate (loses local metadata, not Drive files).
  - *Severity:* High.
